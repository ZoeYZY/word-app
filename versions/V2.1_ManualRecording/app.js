...
const AudioDB = {
    ...
    async save(word, dataUrl) {
        const db = await this.open();
        return new Promise((resolve, reject) => {
            const tx = db.transaction('audio', 'readwrite');
            tx.objectStore('audio').put({ word, dataUrl, ts: Date.now() });
            tx.oncomplete = () => { audioCache[word] = dataUrl; resolve(); };
    ...
    async get(word) {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = () => { const r = req.result; if (r) audioCache[word] = r.dataUrl; resolve(r ? r.dataUrl : null); };
            req.onerror = () => reject(req.error);
        });
    },
    async delete(word) {
        ...
        return new Promise((resolve, reject) => {
            const tx = db.transaction('audio', 'readwrite');
            tx.objectStore('audio').delete(word);
            tx.oncomplete = () => { delete audioCache[word]; resolve(); };
            tx.onerror = () => reject(tx.error);
        });
    },
    async loadAll() {
...
function showBatchRecWord() {
    ...
    const { lesson, index } = batchRecState;
    ...
    const hasAudio = !!audioCache[word];
    ...
    if (document.getElementById('batchPlayBtn')) {
        document.getElementById('batchPlayBtn').onclick = () => { if (audioCache[word]) playAudioDataUrl(audioCache[word]); };
    }
    if (document.getElementById('batchDelBtn')) {
        document.getElementById('batchDelBtn').onclick = async () => { await AudioDB.delete(word); showBatchRecWord(); };
    }
...
function rebuildWordsDiv(wordsDiv, lesson, mm) {
    ...
    lesson.words.forEach((word, wi) => {
        ...
        mic.title = audioCache[word] ? '有录音 · 点击试听 · 双击重录' : '点击录音';
        mic.onclick = (e) => { e.stopPropagation(); playOrRecordWord(word, mic); };
        mic.ondblclick = (e) => { e.stopPropagation(); toggleWordRecording(word, mic); };
...
async function speakWord(text) {
    // Prefer custom recorded audio
    const customAudio = await AudioDB.get(text);
    if (customAudio) {
        try { await playAudioDataUrl(customAudio); return; } catch (e) { console.warn('Custom audio playback failed, falling back to TTS'); }
    }
    // Fallback to TTS
    if (!('speechSynthesis' in window)) return;
    window.speechSynthesis.cancel();
    const u = new SpeechSynthesisUtterance(text);
    u.lang = 'zh-CN'; u.rate = 0.75; u.pitch = 1.1;
...
function showMainApp() {
    document.getElementById('authPage').classList.add('hidden');
    document.getElementById('mainApp').classList.remove('hidden');
    document.getElementById('welcomeUser').textContent = session?.user?.email || '家长';
    // Default tab
    switchTab('dictation');
    loadData();
}
...
async function renderHistory() {
    ...
    for (const name in byLesson) {
      ...
      <div class="flex flex-wrap gap-3">${chars.map(m => {
            const dates = m.dates.slice(-5).reverse();
          ...
          <div class="flex flex-wrap gap-1 mt-1.5 justify-center">${dates.map(d => `<span class="date-tag">${d.slice(5)}</span>`).join('')}</div>
